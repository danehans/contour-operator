...
// tlsSecretsAsNsNames returns a list of TLS secrets for the provided
// contour represented as namespace/secret name.
func (r *Reconciler) tlsSecretsAsNsNames(ctx context.Context, contour *operatorv1alpha1.Contour) ([]string, error) {
	...
	for _, s := range current.Items {
		if s.Type == corev1.SecretTypeTLS {
			secrets = append(secrets, fmt.Sprintf("%s/%s", s.Namespace, s.Name))
		}
	}
	return secrets, nil
}
